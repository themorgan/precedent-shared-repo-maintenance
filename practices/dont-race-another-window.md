...
## Story
2026-09-11, same thread as [`handoff-only-when-blocked`](https://github.com/themorgan/precedent-individual/blob/main/practices/handoff-only-when-blocked.md), immediately after the exchange that produced it. Morgan had already pasted the handoff document into other sessions by the time he asked this one to fix the item it should not have handed off. The session noticed the collision, said so — and carried on with the work regardless. That is not what he asked for, and the rule above is written in the shape of the miss: the notice is not the compliance.
