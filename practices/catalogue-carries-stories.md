...
## Install
Checked mechanically by [`tools/checks/check_catalogue_stories.py`](../tools/checks/check_catalogue_stories.py), scope `tree`: it reads every `practices/*.md`, skips any whose `status:` is not `active`, and fails on an empty or whitespace-only `## Story`, naming each one. It deliberately does not judge whether a Story is a *good* incident -- that is not a property a script can test -- only that the section says something. Two-direction tested in [`tools/checks/tests/test_catalogue_stories.sh`](../tools/checks/tests/test_catalogue_stories.sh).
